...
import { generateLeaveAccrualStatementText } from '../utils/calculator';
import {
  exportLeaveAccrualToExcel,
  exportLeaveAccrualToPDF,
} from '../utils/leaveAccrualExport';

interface LeaveAccrualExportModalProps {
  isOpen: boolean;
...
export const LeaveAccrualExportModal: React.FC<LeaveAccrualExportModalProps> = ({
  isOpen,
  inputs,
  results,
  onClose,
  ...
  const handleExcelExport = () => {
    try {
      ...
      exportLeaveAccrualToExcel(inputs, results);
    } catch (err) {
  ...
  const handlePDFExport = () => {
    try {
      ...
      exportLeaveAccrualToPDF(inputs, results);
    } catch (err) {
      ...
      <div className="bg-white dark:bg-zinc-900 rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] flex flex-col overflow-hidden animate-scaleUp transition-colors border border-orange-200/50 dark:border-zinc-800">
        ...
        <div className="bg-orange-600 dark:bg-zinc-800 text-white px-5 py-4 flex items-center justify-between">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h3 className="text-base sm:text-lg font-bold">Export Leave Accrual Workpaper</h3>
              <p className="text-[11px] text-orange-100 dark:text-zinc-400">
                {inputs.employeeName || 'John Smith'} • {inputs.profile}
              </p>
            </div>
          </div>
          <button
        ...
        {/* Footer Actions */}
        <div className="p-3.5 sm:p-4 bg-zinc-50 dark:bg-zinc-900 border-t border-zinc-200 dark:border-zinc-800 flex items-center justify-between gap-2.5">
          <button
            type="button"
            onClick={handlePrintStatement}
            className="flex items-center gap-1.5 px-3.5 py-2 text-xs sm:text-sm font-semibold rounded-xl border border-zinc-300 dark:border-zinc-700 bg-white dark:bg-zinc-800 text-zinc-800 dark:text-zinc-200 hover:bg-zinc-100 dark:hover:bg-zinc-700 transition-colors cursor-pointer"
          >
          ...
          <button
            ...
            className={`flex items-center gap-1.5 px-4 py-2 text-xs sm:text-sm font-bold rounded-xl shadow-xs transition-all cursor-pointer ${
              copied
