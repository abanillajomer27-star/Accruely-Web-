...
import { generateStandardOTAdjustmentStatementText } from '../utils/calculator';
import {
  exportStandardOTToExcel,
  exportStandardOTToPDF,
} from '../utils/standardOTExport';

interface StandardOTAdjustmentExportModalProps {
  isOpen: boolean;
...
export const StandardOTAdjustmentExportModal: React.FC<
  StandardOTAdjustmentExportModalProps
> = ({ isOpen, inputs, results, onClose }) => {
  ...
  const handleExcelExport = () => {
    try {
      ...
      exportStandardOTToExcel(inputs, results);
    } catch (err) {
  ...
  const handlePDFExport = () => {
    try {
      ...
      exportStandardOTToPDF(inputs, results);
    } catch (err) {
      ...
      <div className="bg-white dark:bg-zinc-900 rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] flex flex-col overflow-hidden animate-scaleUp transition-colors border border-orange-200/50 dark:border-zinc-800">
        ...
        <div className="bg-orange-600 dark:bg-zinc-800 text-white px-5 py-4 flex items-center justify-between">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h3 className="text-base sm:text-lg font-bold">Export Standard OT Workpaper</h3>
              <p className="text-[11px] text-orange-100 dark:text-zinc-400">
                {inputs.employeeName || 'John Smith'} • Prorated Overtime with LWOP
              </p>
            </div>
          </div>
          <button
